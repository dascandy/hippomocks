...
class ref_tuple<Arg, Args...> : public base_tuple
{
public:
	Arg head;
	ref_tuple<Args...> tail;
	ref_tuple(Arg arg, Args... args)
		  : head(arg), tail(args...)
	{
	}
	template <typename RV, typename T, typename... N>
...
template <typename... Args>
class ref_comparable_tuple
{
public:
	virtual bool operator==(const ref_tuple<Args...> &to) = 0;
};
...
{
public:
	virtual bool operator==(const ref_tuple<> &to) = 0;
};

template <typename Arg, typename... Args>
class ref_comparable_tuple<Arg, Args...> : public base_tuple
{
public:
    typedef Arg head_type;
    typedef ref_comparable_tuple<Args...> child_type;
    typedef ref_tuple<Arg, Args...> compare_type;
...
class copy_tuple<ref> : public ref
{
public:
	bool operator==(const ref_tuple<> &)
	{
		return true;
	}
	virtual void printTo(std::ostream &os, bool printComma = false) const
...
template <typename ref, typename Arg, typename... Args>
...
public:
	...
	bool operator==(const typename ref::compare_type &to)
	{
		return (comparer<typename ref::head_type>::compare(head, to.head) && tail == to.tail);
	}
	virtual void printTo(std::ostream &os, bool printComma = false) const
	{
		if (!printComma)
...
template <typename... Args>
struct  HasNoArgs { enum { value = false }; };

template <>
struct  HasNoArgs<> { enum { value = true }; };
...
public:
	...
	template <typename T>
	Call &Do(T &function) { functor = new DoWrapper<T,void>(function); return *this; }
	template <typename Ex>
	Call &Throw(Ex exception) { eHolder = new ExceptionWrapper<Ex>(exception); return *this; }
};
